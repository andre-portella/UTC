...
class CBSQ(AL):
    ...
    def turtle_uncert(self, labeled, scores, strategy, labels_org, labels_pse, n_query):
        if self.idx != 0:
            label_len = labeled.size()[0]
            # concat_tensor = torch.cat([labeled, unlabeled], dim=0)
            ...
            label_len = 0
            # concat_tensor = unlabeled
            thresholds = {c: 99 for c in range(self.n_class)}

        # Build unified feature space for TURTLE        
        if self.idx == 0:
        ...
        if self.idx:
            num_clusters = self.n_class * 4 + label_len
        else:
            num_clusters = self.n_class * 4
        ...
        if self.idx == 0:
            ...
            for c in uniques:
                ...
                features_cluster = torch.from_numpy(train_features[unlabeled_in_cluster]).float()
                labels_cluster = torch.from_numpy(train_labels[unlabeled_in_cluster]).float()

                probs_cluster = probs[unlabeled_in_cluster]
                Q_cluster_cand, _ = selector.run(strategy= strategy, n_query=budget_per_clusters[c], features_cluster=features_cluster, probs_cluster=probs_cluster)
                ...
                for local_rank in Q_cluster_cand:
                    ...
                    if scores[idx] >= thresholds[labels_pse[idx]]:
                        if labels_pse[idx] == labels_org[idx]:
                            corr += 1
                        budget_saving += 1
        ...
        if budget_saving == 0:
            ...
            corr_ratio = corr / budget_saving

        return final_indices.cpu().numpy(), budget_saving, corr_ratio, cluster_acc_train, cluster_acc_val, preds_train, preds_test
    ...
    def select(self, n_query, **kwargs):
        ...
        selected_indices, budget_saving, corr_ratio, cluster_acc_train, cluster_acc_val, preds_train, preds_test = self.turtle_uncert(labeled_features, scores, self.cfg.STRATEGY, labels_org, labels_pse, n_query)
        Q_index = [(self.U_index[int(idx)], int(val), int(flag)) for idx, val, flag in selected_indices]


        # labeled + unlabeled
        train_features_base = torch.cat([labeled_features, unlabeled_features], dim=0)
        ...
        return Q_index, budget_saving, corr_ratio, cluster_acc_train, cluster_acc_val