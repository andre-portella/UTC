...
def _parse_args(args):
    ...
    parser.add_argument('--clusters', type=int, default=None, help="Number of clusters for Task Encoder")
    parser.add_argument('--strategy', type=str, default="centroid", help="AL strategy for sample selection", choices=["centroid", "entropy", "confidence", "margin", "margin_confidence", "kdn_entropy"])
    parser.add_argument('--round', type=int, default=None, help="Round number of active learning")
...
def run(args=None):
    ...
    Zs_train = [np.load(f"{args.root_dir}/representations/{phi}/{args.dataset}/train_{args.strategy}_round{args.round}_seed{args.seed}.npy").astype(np.float32) for phi in args.phis]
    Zs_val = [np.load(f"{args.root_dir}/representations/{phi}/{args.dataset}/val_{args.strategy}_round{args.round}_seed{args.seed}.npy").astype(np.float32) for phi in args.phis]
    y_gt_val = np.load(f"{args.root_dir}/labels/{args.dataset}/val_{args.strategy}_round{args.round}_seed{args.seed}.npy")
    y_gt_train = np.load(f"{args.root_dir}/labels/{args.dataset}/train_{args.strategy}_round{args.round}_seed{args.seed}.npy")
    ...
    for i in iters_bar:
        ...
        if (i+1) % 20 == 0 or (i+1) == args.T:
            ...
            # train
            labels_train, _ = task_encoding(
                [torch.from_numpy(Z_train).to(args.device) for Z_train in Zs_train]
            )

            preds_train = labels_train.argmax(dim=1).detach().cpu().numpy()

            cluster_acc_train, _ = get_cluster_acc(preds_train, y_gt_train)
            ...
            cluster_acc_val, _ = get_cluster_acc(preds_val, y_gt_val)

            iters_bar.set_description(
                f"Train Acc {cluster_acc_train:.4f}, "
                f"Val Acc {cluster_acc_val:.4f}, "
                f"clusters {len(np.unique(preds_val))}/{C}"
            )
    ...
    print(f'Val finished! ')
    print(f'Val loss {float(pred_error):.3f}, entropy {float(entr_reg):.3f}, Number of found clusters {len(np.unique(preds_val))}/{C}, Cluster Acc {cluster_acc_val:.4f}')
    ...
    task_path = f"turtle_{phis}_innerlr{args.inner_lr}_outerlr{args.outer_lr}_T{args.T}_M{args.M}_{inner_start}_gamma{args.gamma}_bs{args.batch_size}_{args.strategy}_round{args.round}_seed{args.seed}" 
    ...
    num_spaces = len(args.phis)
    phis = '_'.join(args.phis)
    metrics_dir = f"{args.root_dir}/results/{num_spaces}space/{phis}/{args.dataset}"
    os.makedirs(metrics_dir, exist_ok=True)


    csv_file_path = f"{metrics_dir}/metrics_{args.strategy}_seed{args.seed}.csv"
    num_found_clusters = len(np.unique(preds_val))

    # Determine the mode for writing to the CSV file
    # If the file doesn't exist or if it's the first round, we write the header; otherwise, we append to the existing file.
    mode = 'w' if (args.round == 0 or not os.path.exists(csv_file_path)) else 'a'

    with open(csv_file_path, mode, newline='') as csv_file:
        csv_writer = csv.writer(csv_file)
        
        # Write the header only if we're in write mode (i.e., first round or file doesn't exist)
        if mode == 'w':
            csv_writer.writerow(['round', 'execution_time', 'val_loss', 'entropy', 'num_clusters', 'acc_clusters_train', 'acc_clusters_val'])
        
        csv_writer.writerow([
            args.round,
            f"{execution_time:.2f}",
            f"{float(pred_error):.4f}",
            f"{float(entr_reg):.4f}",
            num_found_clusters,
            f"{cluster_acc_train:.4f}",
            f"{cluster_acc_val:.4f}"
        ])
    
    return cluster_acc_train, cluster_acc_val, preds_train, preds_val
